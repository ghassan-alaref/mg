import React, { useState, useEffect } from "react";
import { Link, NavLink, useLocation } from "react-router-dom";
import Logo from "../Images/logo.png";
import NewCustomersIcon from "../Images/Vector.png";
import CollectionsIcon from "../Images/wallet-money.png";
import LanguageIcon from "../Images/language.png";
import UserIcon from "../Images/user-square.png";
import Arrow from "../Images/next.png";
import { useTranslation } from "react-i18next";
import { changeLanguage } from "../Language/languageUtils";
import { useCookies } from "react-cookie";
import { useCustomerOptions } from "../Context/CustomerOptionsContext";
import { useAppContext } from "../Context/NewCustomerContext";
import { useCollectionContext } from "../Context/CollectionProvider";
import { useCustomerInfoContext } from "../Context/CustomerInfoContext";

const Sidebar = () => {
  const [cookies] = useCookies(["token"]);
  const { logoutCustomerOptions } = useCustomerOptions();
  const { logoutContext } = useAppContext();
  const { logoutCollectionContext } = useCollectionContext();
  const { logoutCustomerInfo } = useCustomerInfoContext();
  const location = useLocation();
  //const params = new URLSearchParams(location.search);
  const [dbName, setDbName] = useState(cookies.token?.selectedCompany);
  const [token, setToken] = useState(cookies.token?.token);

  useEffect(() => {
    const storedDbName = localStorage.getItem("dbName");
    const storedToken = localStorage.getItem("token");
    if (!dbName && storedDbName) setDbName(storedDbName);
    if (!token && storedToken) setToken(storedToken);
  }, [dbName, token]);

  useEffect(() => {
    if (dbName) localStorage.setItem("dbName", dbName);
    if (token) localStorage.setItem("token", token);
  }, [dbName, token]);

  function navToHome() {
    logoutCustomerOptions();
    logoutContext();
    logoutCollectionContext();
    logoutCustomerInfo();
  }
  function navToNewCustomer() {
    logoutCustomerOptions();
    logoutContext();
    logoutCollectionContext();
  }
  function navToCollection() {
    logoutCustomerOptions();
    logoutContext();
  }
  function navToPendingCustomers() {
    logoutCustomerOptions();
    logoutContext();
  }  

  const isHomeRoute =
    location.pathname.startsWith("/home") ||
    location.pathname.startsWith("/customer-info") ||
    location.pathname.startsWith("/customeroptions");
  const isNewcustomers = location.pathname === "/new-customers";
  const isCollectionsRoute = location.pathname === "/collections";
  const isPendingRoute = location.pathname === "/pendingCustomers";

  const { i18n, t } = useTranslation();
  const [isSidebarOpen, setIsSidebarOpen] = useState(false); // State for sidebar toggle on mobile
  const [isLanguageDropdownOpen, setIsLanguageDropdownOpen] = useState(false);

  const toggleLanguageDropdown = () => {
    setIsLanguageDropdownOpen(!isLanguageDropdownOpen);
  };

  const handleLanguageChange = (lng) => {
    changeLanguage(lng);
    setIsLanguageDropdownOpen(false);
  };

  const direction = i18n.language === "ar" ? "rtl" : "ltr";

  return (
    <div className="flex md:min-h-screen" dir={direction}>
      {/* <div className={`p-4 md:hidden ${isSidebarOpen ? 'absolute z-50 top-0 right-0' : ''}`}>
                <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="flex items-center justify-center w-8 h-8 mt-4">
                    <img src={MenuIcon} alt="Menu" className="h-6 w-6" />
                </button>
            </div> */}

      <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white shadow-lg z-50">
        <nav className="flex justify-around text-center">
          {/* Home Tab */}
          <NavLink
            to={`/home`}
            className={({ isActive }) =>
              `flex flex-col items-center py-3 w-full ${
                isActive ? "bg-[#272C6F]" : ""
              }`
            }
          >
            <img src={UserIcon} alt="Home" className="h-6 mb-1" />
            <p className="font-semibold" style={{ fontSize: "0.55rem" }}>
              {t("customers")}
            </p>
          </NavLink>

          {/* Search Tab */}
          {/* Search Tab */}
          <NavLink
            to={`/new-customers`}
            className={({ isActive }) =>
              `flex flex-col items-center py-3 w-full ${
                isActive ? "bg-[#272C6F]" : ""
              }`
            }
          >
            <img src={NewCustomersIcon} alt="Search" className="h-6 mb-1" />
            <p className="font-semibold" style={{ fontSize: "0.55rem" }}>
              {t("newCustomers")}
            </p>
          </NavLink>

          {/* Profile Tab */}
          <NavLink
            to={`/collections`}
            end
            className={({ isActive }) =>
              `flex flex-col items-center py-3 w-full ${
                isActive ? "bg-[#272C6F]" : ""
              }`
            }
          >
            <img src={CollectionsIcon} alt="Profile" className="h-6 mb-1" />
            <p className="font-semibold" style={{ fontSize: "0.55rem" }}>
              {t("Collections")}
            </p>
          </NavLink>

          <NavLink
            to={`/pendingCustomers`}
            end
            className={({ isActive }) =>
              `flex flex-col items-center py-3 w-full ${
                isActive ? "bg-[#272C6F]" : ""
              }`
            }
          >
            <img src={CollectionsIcon} alt="Icon" className="h-6 mb-1" />
            <p className="font-semibold" style={{ fontSize: "0.55rem" }}>
              {t("PendingCustomers")}
            </p>
          </NavLink>

          {/* Settings Tab */}
          <div
            className="relative flex flex-col items-center py-3 w-full"
            onClick={toggleLanguageDropdown}
          >
            <img src={LanguageIcon} alt="Language" className="h-6 mb-1" />
            <p className="font-semibold" style={{ fontSize: "0.55rem" }}>
              {t("language")}
            </p>

            {/* Language Dropdown Menu */}
            {isLanguageDropdownOpen && (
              <div
                className={`absolute bottom-8  mb-14 w-full bg-white shadow-md rounded-lg py-1 z-10 ${
                  i18n.language === "ar" ? "ml-3 md:ml-0" : "mr-3 md:mr-0"
                }`}
              >
                <ul>
                  <li
                    className="text-xs px-0 py-2 hover:bg-blue-100 cursor-pointer"
                    onClick={() => handleLanguageChange("ar")}
                  >
                    العربية
                  </li>
                  <li
                    className="text-xs px-0 py-2 hover:bg-blue-100 cursor-pointer"
                    onClick={() => handleLanguageChange("en")}
                  >
                    English
                  </li>
                </ul>
              </div>
            )}
          </div>
        </nav>
      </div>

      {/* Sidebar */}
      <div
        className={`bg-white p-5 flex flex-col justify-between ${
          isSidebarOpen ? "fixed inset-0 z-40 overflow-y-auto" : "hidden"
        } md:flex md:relative md:w-52 lg:w-72`}
      >
        {/* Ensuring the sidebar content is scrollable on mobile devices if it overflows */}
        <div>
          {/* Logo */}
          <Link
            to={`/home`}
            className="flex justify-center items-center"
            onClick={navToHome}
          >
            <img src={Logo} alt="Logo" className="h-12 mb-10" />
          </Link>

          {/* Navigation */}
          <nav>
            {/* NavLink for "Customers" */}
            <NavLink
              to={`/home`}
              className={`flex  items-center w-1/2 md:w-full pl-4 py-3 mt-3 transition duration-200 ease-in-out transform hover:translate-x-1 hover:bg-blue-100 rounded-md ${
                isHomeRoute ? "bg-[#272C6F]" : ""
              }`}
              onClick={navToHome}
            >
              <img src={UserIcon} alt="Customers" className="h-5 md:h-6 mr-3" />
              <div className="flex flex-row justify-between items-center w-72 md:w-full">
                <p
                  className={`text-center text-xs md:text-xs lg:text-sm font-semibold ${
                    i18n.language === "ar" ? "mr-5" : ""
                  }`}
                  style={{ color: isHomeRoute ? "#FFFFFF" : "#489AB9" }}
                >
                  {t("customers")}
                </p>

                <img
                  src={Arrow}
                  className={`w-3 h-auto md:w-4 h-auto mt-1 ${
                    i18n.language === "ar" ? "rotate-180" : "mr-2"
                  }`}
                />
              </div>
            </NavLink>
            {/* Additional NavLinks follow a similar pattern */}
            <NavLink
              to={`/new-customers`}
              className={`flex items-center  w-1/2 md:w-full pl-4 py-3 mt-3 transition duration-200 ease-in-out transform hover:translate-x-1 hover:bg-blue-100 rounded-md ${
                isNewcustomers ? "bg-[#272C6F]" : ""
              }`}
              onClick={navToNewCustomer}
            >
              <img
                src={NewCustomersIcon}
                alt="New Customers"
                className="h-5 md:h-6 mr-3"
              />
              <div className="flex flex-row justify-between items-center  w-72 md:w-full">
                <p
                  className={`text-center text-xs md:text-xs lg:text-sm font-semibold ${
                    i18n.language === "ar" ? "mr-5" : ""
                  }`}
                  style={{ color: isNewcustomers ? "#FFFFFF" : "#489AB9" }}
                >
                  {t("newCustomers")}
                </p>
                <img
                  src={Arrow}
                  className={`w-3 h-auto md:w-4 h-auto mt-1 ${
                    i18n.language === "ar" ? "rotate-180" : "mr-2"
                  }`}
                />
              </div>
            </NavLink>
            <NavLink
              to={`/collections`}
              className={`flex  items-center w-1/2 md:w-full pl-4 py-3 mt-3 transition duration-200 ease-in-out transform hover:translate-x-1 hover:bg-blue-100 rounded-md ${
                isCollectionsRoute ? "bg-[#272C6F]" : ""
              }`}
              onClick={navToCollection}
            >
              <img
                src={CollectionsIcon}
                alt="Collections"
                className="h-5 md:h-6  mr-3"
              />
              <div className="flex flex-row justify-between items-center  w-72 md:w-fulll">
                <p
                  className={`text-center text-xs md:text-xs lg:text-sm font-semibold ${
                    i18n.language === "ar" ? "mr-5" : ""
                  }`}
                  style={{ color: isCollectionsRoute ? "#FFFFFF" : "#489AB9" }}
                >
                  {t("Collections")}
                </p>
                <img
                  src={Arrow}
                  className={`w-3 h-auto md:w-4 h-auto mt-1 ${
                    i18n.language === "ar" ? "rotate-180" : "mr-2"
                  }`}
                />
              </div>
            </NavLink>


            <NavLink
              to={`/pendingCustomers`}
              className={`flex  items-center w-1/2 md:w-full pl-4 py-3 mt-3 transition duration-200 ease-in-out transform hover:translate-x-1 hover:bg-blue-100 rounded-md ${
                isPendingRoute ? "bg-[#272C6F]" : ""
              }`}
              onClick={navToPendingCustomers}
            >
            <img src={UserIcon} alt="Customers" className="h-5 md:h-6 mr-3" />

              <div className="flex flex-row justify-between items-center  w-72 md:w-fulll">
                <p
                  className={`text-center text-xs md:text-xs lg:text-sm font-semibold ${
                    i18n.language === "ar" ? "mr-5" : ""
                  }`}
                  style={{ color: isPendingRoute ? "#FFFFFF" : "#489AB9" }}
                >
                  {t("PendingCustomers")}
                </p>
                <img
                  src={Arrow}
                  className={`w-3 h-auto md:w-4 h-auto mt-1 ${
                    i18n.language === "ar" ? "rotate-180" : "mr-2"
                  }`}
                />
              </div>
            </NavLink>


            <div
              className={`flex items-center w-1/2 md:w-full pl-4 py-3 mt-3 transition duration-200 ease-in-out transform hover:translate-x-1 hover:bg-blue-100 rounded-md ${
                isLanguageDropdownOpen ? "bg-[#272C6F]" : "bg-transparent"
              } cursor-pointer`}
              onClick={toggleLanguageDropdown}
            >
              <img
                src={LanguageIcon}
                alt="Language"
                className="h-5 md:h-6 mr-3"
              />
              <div className="flex flex-row justify-between items-center w-72 md:w-full">
                <p
                  className={`text-center text-xs md:text-xs lg:text-sm font-semibold ${
                    i18n.language === "ar" ? "mr-5" : ""
                  }`}
                  style={{
                    color: isLanguageDropdownOpen ? "#FFFFFF" : "#489AB9",
                  }}
                >
                  {t("language")}
                </p>
                <img
                  src={Arrow}
                  className={`w-3 h-auto md:w-4 h-auto mt-1 ${
                    i18n.language === "ar" ? "rotate-180" : "mr-2"
                  }`}
                />
              </div>
            </div>

            {isLanguageDropdownOpen && (
              <div className="absolute mt-3 w-48 bg-white shadow-md rounded-lg py-1">
                <ul>
                  <li
                    className="px-4 py-2 hover:bg-blue-100 cursor-pointer"
                    onClick={() => handleLanguageChange("ar")}
                  >
                    العربية
                  </li>
                  <li
                    className="px-4 py-2 hover:bg-blue-100 cursor-pointer"
                    onClick={() => handleLanguageChange("en")}
                  >
                    English
                  </li>
                </ul>
              </div>
            )}
          </nav>
        </div>

        {/* User Profile Section */}
        <div className="flex items-center mb-10">
          <div className="flex items-center justify-center h-12 w-12 rounded-full bg-gray-300 mr-3">
            {/* Placeholder for user icon, potentially an <img> or an icon component */}
          </div>
          <span
            className={`text-sm font-semibold ${
              i18n.language === "ar" ? "mr-5" : ""
            }`}
          >
            {t("userProfile")}
          </span>
        </div>
      </div>

      {/* Main Content */}
      <div className={`flex-grow ${isSidebarOpen ? "hidden" : ""}`}>
        {/* Your main content goes here */}
      </div>
    </div>
  );
};

export default Sidebar;
